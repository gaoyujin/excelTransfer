...
def change_run(tip):
    if tip == 'init':
        ...
        print(f"{Fore.LIGHTGREEN_EX}{Style.BRIGHT}输入==》 7 or BV list or BV (PDF等文件信息收集){Style.RESET_ALL}")
    ...
    if std == '6' or std == '图纸目录V2' or std.startswith('目录V2'):
        try:
            ...
            change_run('no')
    if std == '7' or std == 'BV list' or std.startswith('BV'):
        try:
            run('bv')
        except KeyboardInterrupt:
            change_run('no')
    if (std != '1' and std != '2' and std != '3' and std != '4' and std != '5' and std != '6' and std != '7' and
            std != '图纸目录' and std != '清单list' and std != 'CCS特殊字符' and std != 'ABS命名规则' and std != '去除中文名称' and
            std != '目录' and std != '清单' and std != 'CCS' and std != 'ABS' and std != '去除' and
            std != '图纸目录V2' and std != '目录V2' and std != 'BV list' and std != 'BV'):
        is_run = True
